...
public class VAdjIEEEQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          VAdjIEEEBusinessDelegate.getVAdjIEEEInstance()
              .getVAdjIEEE(new VAdjIEEEFetchOneSummary(vAdjIEEEId));
    } catch (Throwable exc) {
  ...
  public List<VAdjIEEE> loadAll() {
    ...
    try {
      ...
      if (vAdjIEEEList != null) LOGGER.log(Level.INFO, "successfully loaded all VAdjIEEEs");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(VAdjIEEEQueryRestController.class.getName());
}
