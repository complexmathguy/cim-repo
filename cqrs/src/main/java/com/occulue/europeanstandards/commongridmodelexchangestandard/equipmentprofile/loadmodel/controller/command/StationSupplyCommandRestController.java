...
public class StationSupplyCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateStationSupplyCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          StationSupplyBusinessDelegate.getStationSupplyInstance().createStationSupply(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateStationSupplyCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          StationSupplyBusinessDelegate.getStationSupplyInstance().updateStationSupply(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "StationSupplyController:update() - successfully update StationSupply - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID stationSupplyId) {
    ...
    try {
      StationSupplyBusinessDelegate delegate =
          StationSupplyBusinessDelegate.getStationSupplyInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted StationSupply with key " + command.getStationSupplyId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(StationSupplyCommandRestController.class.getName());
}
