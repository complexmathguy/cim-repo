...
public class TapChangerQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          TapChangerBusinessDelegate.getTapChangerInstance()
              .getTapChanger(new TapChangerFetchOneSummary(tapChangerId));
    } catch (Throwable exc) {
  ...
  public List<TapChanger> loadAll() {
    ...
    try {
      ...
      if (tapChangerList != null) LOGGER.log(Level.INFO, "successfully loaded all TapChangers");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(TapChangerQueryRestController.class.getName());
}
