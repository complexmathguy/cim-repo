...
public class LoadMotorQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          LoadMotorBusinessDelegate.getLoadMotorInstance()
              .getLoadMotor(new LoadMotorFetchOneSummary(loadMotorId));
    } catch (Throwable exc) {
  ...
  public List<LoadMotor> loadAll() {
    ...
    try {
      ...
      if (loadMotorList != null) LOGGER.log(Level.INFO, "successfully loaded all LoadMotors");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(LoadMotorQueryRestController.class.getName());
}
