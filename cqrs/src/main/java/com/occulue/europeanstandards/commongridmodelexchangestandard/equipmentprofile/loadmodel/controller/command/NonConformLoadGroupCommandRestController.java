...
public class NonConformLoadGroupCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateNonConformLoadGroupCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          NonConformLoadGroupBusinessDelegate.getNonConformLoadGroupInstance()
              .createNonConformLoadGroup(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateNonConformLoadGroupCommand command) {
    ...
    try {
      ...
      completableFuture =
          NonConformLoadGroupBusinessDelegate.getNonConformLoadGroupInstance()
              .updateNonConformLoadGroup(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "NonConformLoadGroupController:update() - successfully update NonConformLoadGroup - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID nonConformLoadGroupId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteNonConformLoadGroupCommand command =
        new DeleteNonConformLoadGroupCommand(nonConformLoadGroupId);

    try {
      NonConformLoadGroupBusinessDelegate delegate =
          NonConformLoadGroupBusinessDelegate.getNonConformLoadGroupInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted NonConformLoadGroup with key "
              + command.getNonConformLoadGroupId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(NonConformLoadGroupCommandRestController.class.getName());
}
