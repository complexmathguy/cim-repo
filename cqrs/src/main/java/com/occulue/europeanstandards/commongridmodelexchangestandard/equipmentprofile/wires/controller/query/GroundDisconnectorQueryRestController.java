...
public class GroundDisconnectorQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GroundDisconnectorBusinessDelegate.getGroundDisconnectorInstance()
              .getGroundDisconnector(new GroundDisconnectorFetchOneSummary(groundDisconnectorId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load GroundDisconnector using Id " + groundDisconnectorId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GroundDisconnectorQueryRestController.class.getName());
}
