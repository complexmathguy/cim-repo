...
public class DCChopperQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DCChopperBusinessDelegate.getDCChopperInstance()
              .getDCChopper(new DCChopperFetchOneSummary(dCChopperId));
    } catch (Throwable exc) {
  ...
  public List<DCChopper> loadAll() {
    ...
    try {
      ...
      if (dCChopperList != null) LOGGER.log(Level.INFO, "successfully loaded all DCChoppers");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCChopperQueryRestController.class.getName());
}
