...
public class EquipmentContainerCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateEquipmentContainerCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          EquipmentContainerBusinessDelegate.getEquipmentContainerInstance()
              .createEquipmentContainer(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateEquipmentContainerCommand command) {
    ...
    try {
      ...
      completableFuture =
          EquipmentContainerBusinessDelegate.getEquipmentContainerInstance()
              .updateEquipmentContainer(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "EquipmentContainerController:update() - successfully update EquipmentContainer - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID equipmentContainerId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteEquipmentContainerCommand command =
        new DeleteEquipmentContainerCommand(equipmentContainerId);

    try {
      EquipmentContainerBusinessDelegate delegate =
          EquipmentContainerBusinessDelegate.getEquipmentContainerInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted EquipmentContainer with key " + command.getEquipmentContainerId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EquipmentContainerCommandRestController.class.getName());
}
