...
public class LoadAreaCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateLoadAreaCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateLoadAreaCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "LoadAreaController:update() - successfully update LoadArea - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID loadAreaId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted LoadArea with key " + command.getLoadAreaId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(LoadAreaCommandRestController.class.getName());
}
