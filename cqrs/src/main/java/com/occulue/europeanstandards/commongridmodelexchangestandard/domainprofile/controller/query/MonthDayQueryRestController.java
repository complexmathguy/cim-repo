...
public class MonthDayQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          MonthDayBusinessDelegate.getMonthDayInstance()
              .getMonthDay(new MonthDayFetchOneSummary(monthDayId));
    } catch (Throwable exc) {
  ...
  public List<MonthDay> loadAll() {
    ...
    try {
      ...
      if (monthDayList != null) LOGGER.log(Level.INFO, "successfully loaded all MonthDays");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(MonthDayQueryRestController.class.getName());
}
