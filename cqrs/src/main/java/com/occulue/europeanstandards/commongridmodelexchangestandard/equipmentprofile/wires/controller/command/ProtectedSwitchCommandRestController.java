...
public class ProtectedSwitchCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateProtectedSwitchCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          ProtectedSwitchBusinessDelegate.getProtectedSwitchInstance()
              .createProtectedSwitch(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateProtectedSwitchCommand command) {
    ...
    try {
      ...
      completableFuture =
          ProtectedSwitchBusinessDelegate.getProtectedSwitchInstance()
              .updateProtectedSwitch(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "ProtectedSwitchController:update() - successfully update ProtectedSwitch - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID protectedSwitchId) {
    ...
    try {
      ProtectedSwitchBusinessDelegate delegate =
          ProtectedSwitchBusinessDelegate.getProtectedSwitchInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted ProtectedSwitch with key " + command.getProtectedSwitchId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ProtectedSwitchCommandRestController.class.getName());
}
