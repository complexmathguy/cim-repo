...
public class CapacitanceQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          CapacitanceBusinessDelegate.getCapacitanceInstance()
              .getCapacitance(new CapacitanceFetchOneSummary(capacitanceId));
    } catch (Throwable exc) {
  ...
  public List<Capacitance> loadAll() {
    ...
    try {
      ...
      if (capacitanceList != null) LOGGER.log(Level.INFO, "successfully loaded all Capacitances");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(CapacitanceQueryRestController.class.getName());
}
