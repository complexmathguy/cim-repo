...
public class PositionPointQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          PositionPointBusinessDelegate.getPositionPointInstance()
              .getPositionPoint(new PositionPointFetchOneSummary(positionPointId));
    } catch (Throwable exc) {
  ...
  public List<PositionPoint> loadAll() {
    ...
    try {
      ...
      positionPointList =
          PositionPointBusinessDelegate.getPositionPointInstance().getAllPositionPoint();
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PositionPointQueryRestController.class.getName());
}
