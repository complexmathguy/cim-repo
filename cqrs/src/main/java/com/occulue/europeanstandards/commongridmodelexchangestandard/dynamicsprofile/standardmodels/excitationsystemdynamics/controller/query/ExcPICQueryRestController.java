...
public class ExcPICQueryRestController extends BaseSpringRestController {
  ...
  public ExcPIC load(@RequestParam(required = true) UUID excPICId) {
    ...
    try {
      entity =
          ExcPICBusinessDelegate.getExcPICInstance().getExcPIC(new ExcPICFetchOneSummary(excPICId));
    } catch (Throwable exc) {
  ...
  public List<ExcPIC> loadAll() {
    ...
    try {
      ...
      if (excPICList != null) LOGGER.log(Level.INFO, "successfully loaded all ExcPICs");
    } catch (Throwable exc) {
