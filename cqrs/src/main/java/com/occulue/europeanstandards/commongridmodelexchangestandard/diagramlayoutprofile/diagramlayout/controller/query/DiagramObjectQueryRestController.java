...
public class DiagramObjectQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DiagramObjectBusinessDelegate.getDiagramObjectInstance()
              .getDiagramObject(new DiagramObjectFetchOneSummary(diagramObjectId));
    } catch (Throwable exc) {
  ...
  public List<DiagramObject> loadAll() {
    ...
    try {
      ...
      diagramObjectList =
          DiagramObjectBusinessDelegate.getDiagramObjectInstance().getAllDiagramObject();
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramObjectQueryRestController.class.getName());
}
