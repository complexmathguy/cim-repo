...
public class WindGeneratingUnitQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          WindGeneratingUnitBusinessDelegate.getWindGeneratingUnitInstance()
              .getWindGeneratingUnit(new WindGeneratingUnitFetchOneSummary(windGeneratingUnitId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load WindGeneratingUnit using Id " + windGeneratingUnitId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(WindGeneratingUnitQueryRestController.class.getName());
}
