...
public class PhaseTapChangerLinearQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          PhaseTapChangerLinearBusinessDelegate.getPhaseTapChangerLinearInstance()
              .getPhaseTapChangerLinear(
                  new PhaseTapChangerLinearFetchOneSummary(phaseTapChangerLinearId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load PhaseTapChangerLinear using Id " + phaseTapChangerLinearId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerLinearQueryRestController.class.getName());
}
