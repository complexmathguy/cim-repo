...
public class DateProxyCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDateProxyCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDateProxyCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "DateProxyController:update() - successfully update DateProxy - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID dateProxyId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted DateProxy with key " + command.getDateProxyId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DateProxyCommandRestController.class.getName());
}
