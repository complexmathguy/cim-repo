...
public class BooleanProxyCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateBooleanProxyCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          BooleanProxyBusinessDelegate.getBooleanProxyInstance().createBooleanProxy(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateBooleanProxyCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          BooleanProxyBusinessDelegate.getBooleanProxyInstance().updateBooleanProxy(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "BooleanProxyController:update() - successfully update BooleanProxy - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID booleanProxyId) {
    ...
    try {
      BooleanProxyBusinessDelegate delegate =
          BooleanProxyBusinessDelegate.getBooleanProxyInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted BooleanProxy with key " + command.getBooleanProxyId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(BooleanProxyCommandRestController.class.getName());
}
