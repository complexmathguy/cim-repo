...
public class DCBreakerCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDCBreakerCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDCBreakerCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "DCBreakerController:update() - successfully update DCBreaker - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID dCBreakerId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted DCBreaker with key " + command.getDCBreakerId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCBreakerCommandRestController.class.getName());
}
