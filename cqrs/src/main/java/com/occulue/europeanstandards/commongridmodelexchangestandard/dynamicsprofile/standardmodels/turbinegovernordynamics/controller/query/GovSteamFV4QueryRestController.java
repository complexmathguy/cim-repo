...
public class GovSteamFV4QueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovSteamFV4BusinessDelegate.getGovSteamFV4Instance()
              .getGovSteamFV4(new GovSteamFV4FetchOneSummary(govSteamFV4Id));
    } catch (Throwable exc) {
  ...
  public List<GovSteamFV4> loadAll() {
    ...
    try {
      ...
      if (govSteamFV4List != null) LOGGER.log(Level.INFO, "successfully loaded all GovSteamFV4s");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovSteamFV4QueryRestController.class.getName());
}
