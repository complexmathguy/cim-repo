...
public class AccumulatorQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          AccumulatorBusinessDelegate.getAccumulatorInstance()
              .getAccumulator(new AccumulatorFetchOneSummary(accumulatorId));
    } catch (Throwable exc) {
  ...
  public List<Accumulator> loadAll() {
    ...
    try {
      ...
      if (accumulatorList != null) LOGGER.log(Level.INFO, "successfully loaded all Accumulators");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(AccumulatorQueryRestController.class.getName());
}
