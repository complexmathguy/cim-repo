...
public class SwitchProxyQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          SwitchProxyBusinessDelegate.getSwitchProxyInstance()
              .getSwitchProxy(new SwitchProxyFetchOneSummary(switchProxyId));
    } catch (Throwable exc) {
  ...
  public List<SwitchProxy> loadAll() {
    ...
    try {
      ...
      if (switchProxyList != null) LOGGER.log(Level.INFO, "successfully loaded all SwitchProxys");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(SwitchProxyQueryRestController.class.getName());
}
