...
public class LoadStaticQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          LoadStaticBusinessDelegate.getLoadStaticInstance()
              .getLoadStatic(new LoadStaticFetchOneSummary(loadStaticId));
    } catch (Throwable exc) {
  ...
  public List<LoadStatic> loadAll() {
    ...
    try {
      ...
      if (loadStaticList != null) LOGGER.log(Level.INFO, "successfully loaded all LoadStatics");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(LoadStaticQueryRestController.class.getName());
}
