...
public class DiagramObjectStyleCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDiagramObjectStyleCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          DiagramObjectStyleBusinessDelegate.getDiagramObjectStyleInstance()
              .createDiagramObjectStyle(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDiagramObjectStyleCommand command) {
    ...
    try {
      ...
      completableFuture =
          DiagramObjectStyleBusinessDelegate.getDiagramObjectStyleInstance()
              .updateDiagramObjectStyle(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "DiagramObjectStyleController:update() - successfully update DiagramObjectStyle - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID diagramObjectStyleId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteDiagramObjectStyleCommand command =
        new DeleteDiagramObjectStyleCommand(diagramObjectStyleId);

    try {
      DiagramObjectStyleBusinessDelegate delegate =
          DiagramObjectStyleBusinessDelegate.getDiagramObjectStyleInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted DiagramObjectStyle with key " + command.getDiagramObjectStyleId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramObjectStyleCommandRestController.class.getName());
}
