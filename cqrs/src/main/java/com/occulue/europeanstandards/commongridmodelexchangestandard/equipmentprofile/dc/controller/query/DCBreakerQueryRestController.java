...
public class DCBreakerQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DCBreakerBusinessDelegate.getDCBreakerInstance()
              .getDCBreaker(new DCBreakerFetchOneSummary(dCBreakerId));
    } catch (Throwable exc) {
  ...
  public List<DCBreaker> loadAll() {
    ...
    try {
      ...
      if (dCBreakerList != null) LOGGER.log(Level.INFO, "successfully loaded all DCBreakers");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCBreakerQueryRestController.class.getName());
}
