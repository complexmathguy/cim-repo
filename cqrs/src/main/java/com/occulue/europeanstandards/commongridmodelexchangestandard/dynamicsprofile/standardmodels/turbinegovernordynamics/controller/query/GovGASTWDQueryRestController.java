...
public class GovGASTWDQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovGASTWDBusinessDelegate.getGovGASTWDInstance()
              .getGovGASTWD(new GovGASTWDFetchOneSummary(govGASTWDId));
    } catch (Throwable exc) {
  ...
  public List<GovGASTWD> loadAll() {
    ...
    try {
      ...
      if (govGASTWDList != null) LOGGER.log(Level.INFO, "successfully loaded all GovGASTWDs");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovGASTWDQueryRestController.class.getName());
}
