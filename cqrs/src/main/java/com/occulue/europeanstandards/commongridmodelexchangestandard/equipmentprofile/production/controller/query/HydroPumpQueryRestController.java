...
public class HydroPumpQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          HydroPumpBusinessDelegate.getHydroPumpInstance()
              .getHydroPump(new HydroPumpFetchOneSummary(hydroPumpId));
    } catch (Throwable exc) {
  ...
  public List<HydroPump> loadAll() {
    ...
    try {
      ...
      if (hydroPumpList != null) LOGGER.log(Level.INFO, "successfully loaded all HydroPumps");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(HydroPumpQueryRestController.class.getName());
}
