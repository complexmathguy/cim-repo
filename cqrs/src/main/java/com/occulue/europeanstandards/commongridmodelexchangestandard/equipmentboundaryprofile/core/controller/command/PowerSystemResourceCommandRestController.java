...
public class PowerSystemResourceCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreatePowerSystemResourceCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          PowerSystemResourceBusinessDelegate.getPowerSystemResourceInstance()
              .createPowerSystemResource(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdatePowerSystemResourceCommand command) {
    ...
    try {
      ...
      completableFuture =
          PowerSystemResourceBusinessDelegate.getPowerSystemResourceInstance()
              .updatePowerSystemResource(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "PowerSystemResourceController:update() - successfully update PowerSystemResource - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID powerSystemResourceId) {
    CompletableFuture<Void> completableFuture = null;
    DeletePowerSystemResourceCommand command =
        new DeletePowerSystemResourceCommand(powerSystemResourceId);

    try {
      PowerSystemResourceBusinessDelegate delegate =
          PowerSystemResourceBusinessDelegate.getPowerSystemResourceInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted PowerSystemResource with key "
              + command.getPowerSystemResourceId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PowerSystemResourceCommandRestController.class.getName());
}
