...
public class ApparentPowerLimitQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ApparentPowerLimitBusinessDelegate.getApparentPowerLimitInstance()
              .getApparentPowerLimit(new ApparentPowerLimitFetchOneSummary(apparentPowerLimitId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load ApparentPowerLimit using Id " + apparentPowerLimitId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ApparentPowerLimitQueryRestController.class.getName());
}
