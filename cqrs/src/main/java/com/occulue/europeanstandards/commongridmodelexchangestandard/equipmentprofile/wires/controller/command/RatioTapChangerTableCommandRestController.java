...
public class RatioTapChangerTableCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateRatioTapChangerTableCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          RatioTapChangerTableBusinessDelegate.getRatioTapChangerTableInstance()
              .createRatioTapChangerTable(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateRatioTapChangerTableCommand command) {
    ...
    try {
      ...
      completableFuture =
          RatioTapChangerTableBusinessDelegate.getRatioTapChangerTableInstance()
              .updateRatioTapChangerTable(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "RatioTapChangerTableController:update() - successfully update RatioTapChangerTable - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(
      @RequestParam(required = true) UUID ratioTapChangerTableId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteRatioTapChangerTableCommand command =
        new DeleteRatioTapChangerTableCommand(ratioTapChangerTableId);

    try {
      RatioTapChangerTableBusinessDelegate delegate =
          RatioTapChangerTableBusinessDelegate.getRatioTapChangerTableInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted RatioTapChangerTable with key "
              + command.getRatioTapChangerTableId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(RatioTapChangerTableCommandRestController.class.getName());
}
