...
public class RegulationScheduleQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          RegulationScheduleBusinessDelegate.getRegulationScheduleInstance()
              .getRegulationSchedule(new RegulationScheduleFetchOneSummary(regulationScheduleId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load RegulationSchedule using Id " + regulationScheduleId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(RegulationScheduleQueryRestController.class.getName());
}
