...
public class ActivePowerPerFrequencyQueryRestController extends BaseSpringRestController {
   ...
   */
  @GetMapping("/load")
  public ActivePowerPerFrequency load(
    ...
    try {
      entity =
          ActivePowerPerFrequencyBusinessDelegate.getActivePowerPerFrequencyInstance()
              .getActivePowerPerFrequency(
                  new ActivePowerPerFrequencyFetchOneSummary(activePowerPerFrequencyId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load ActivePowerPerFrequency using Id " + activePowerPerFrequencyId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ActivePowerPerFrequencyQueryRestController.class.getName());
}
