 ...
 * ****************************************************************************
 */
package com.occulue.europeanstandards.commongridmodelexchangestandard.diagramlayoutprofile.controller.query;
...
public class DiagramLayoutVersionQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DiagramLayoutVersionBusinessDelegate.getDiagramLayoutVersionInstance()
              .getDiagramLayoutVersion(
                  new DiagramLayoutVersionFetchOneSummary(diagramLayoutVersionId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load DiagramLayoutVersion using Id " + diagramLayoutVersionId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramLayoutVersionQueryRestController.class.getName());
}
