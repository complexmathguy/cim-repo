...
public class EquivalentEquipmentQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          EquivalentEquipmentBusinessDelegate.getEquivalentEquipmentInstance()
              .getEquivalentEquipment(
                  new EquivalentEquipmentFetchOneSummary(equivalentEquipmentId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load EquivalentEquipment using Id " + equivalentEquipmentId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EquivalentEquipmentQueryRestController.class.getName());
}
