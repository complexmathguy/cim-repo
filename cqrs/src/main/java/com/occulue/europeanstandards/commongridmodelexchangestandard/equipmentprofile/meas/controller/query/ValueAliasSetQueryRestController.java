...
public class ValueAliasSetQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ValueAliasSetBusinessDelegate.getValueAliasSetInstance()
              .getValueAliasSet(new ValueAliasSetFetchOneSummary(valueAliasSetId));
    } catch (Throwable exc) {
  ...
  public List<ValueAliasSet> loadAll() {
    ...
    try {
      ...
      valueAliasSetList =
          ValueAliasSetBusinessDelegate.getValueAliasSetInstance().getAllValueAliasSet();
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ValueAliasSetQueryRestController.class.getName());
}
