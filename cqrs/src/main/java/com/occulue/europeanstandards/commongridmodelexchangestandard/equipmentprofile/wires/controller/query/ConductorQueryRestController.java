...
public class ConductorQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ConductorBusinessDelegate.getConductorInstance()
              .getConductor(new ConductorFetchOneSummary(conductorId));
    } catch (Throwable exc) {
  ...
  public List<Conductor> loadAll() {
    ...
    try {
      ...
      if (conductorList != null) LOGGER.log(Level.INFO, "successfully loaded all Conductors");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConductorQueryRestController.class.getName());
}
