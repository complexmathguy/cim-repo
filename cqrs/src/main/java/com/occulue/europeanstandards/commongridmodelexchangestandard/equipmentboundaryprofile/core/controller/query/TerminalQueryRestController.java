...
public class TerminalQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          TerminalBusinessDelegate.getTerminalInstance()
              .getTerminal(new TerminalFetchOneSummary(terminalId));
    } catch (Throwable exc) {
  ...
  public List<Terminal> loadAll() {
    ...
    try {
      ...
      if (terminalList != null) LOGGER.log(Level.INFO, "successfully loaded all Terminals");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(TerminalQueryRestController.class.getName());
}
