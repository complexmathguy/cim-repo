...
public class GovCT2QueryRestController extends BaseSpringRestController {
  ...
  public GovCT2 load(@RequestParam(required = true) UUID govCT2Id) {
    ...
    try {
      entity =
          GovCT2BusinessDelegate.getGovCT2Instance().getGovCT2(new GovCT2FetchOneSummary(govCT2Id));
    } catch (Throwable exc) {
  ...
  public List<GovCT2> loadAll() {
    ...
    try {
      ...
      if (govCT2List != null) LOGGER.log(Level.INFO, "successfully loaded all GovCT2s");
    } catch (Throwable exc) {
