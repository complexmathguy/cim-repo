...
public class ConnectorQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ConnectorBusinessDelegate.getConnectorInstance()
              .getConnector(new ConnectorFetchOneSummary(connectorId));
    } catch (Throwable exc) {
  ...
  public List<Connector> loadAll() {
    ...
    try {
      ...
      if (connectorList != null) LOGGER.log(Level.INFO, "successfully loaded all Connectors");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConnectorQueryRestController.class.getName());
}
