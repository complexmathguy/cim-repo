...
public class LoadAggregateCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateLoadAggregateCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          LoadAggregateBusinessDelegate.getLoadAggregateInstance().createLoadAggregate(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateLoadAggregateCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          LoadAggregateBusinessDelegate.getLoadAggregateInstance().updateLoadAggregate(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "LoadAggregateController:update() - successfully update LoadAggregate - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID loadAggregateId) {
    ...
    try {
      LoadAggregateBusinessDelegate delegate =
          LoadAggregateBusinessDelegate.getLoadAggregateInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted LoadAggregate with key " + command.getLoadAggregateId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(LoadAggregateCommandRestController.class.getName());
}
