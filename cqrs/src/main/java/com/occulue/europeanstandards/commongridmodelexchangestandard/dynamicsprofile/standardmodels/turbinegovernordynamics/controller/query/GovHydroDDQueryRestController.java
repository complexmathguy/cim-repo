...
public class GovHydroDDQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovHydroDDBusinessDelegate.getGovHydroDDInstance()
              .getGovHydroDD(new GovHydroDDFetchOneSummary(govHydroDDId));
    } catch (Throwable exc) {
  ...
  public List<GovHydroDD> loadAll() {
    ...
    try {
      ...
      if (govHydroDDList != null) LOGGER.log(Level.INFO, "successfully loaded all GovHydroDDs");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovHydroDDQueryRestController.class.getName());
}
