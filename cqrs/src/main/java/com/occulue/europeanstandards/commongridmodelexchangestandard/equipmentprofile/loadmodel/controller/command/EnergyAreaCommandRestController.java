...
public class EnergyAreaCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateEnergyAreaCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          EnergyAreaBusinessDelegate.getEnergyAreaInstance().createEnergyArea(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateEnergyAreaCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          EnergyAreaBusinessDelegate.getEnergyAreaInstance().updateEnergyArea(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "EnergyAreaController:update() - successfully update EnergyArea - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID energyAreaId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted EnergyArea with key " + command.getEnergyAreaId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EnergyAreaCommandRestController.class.getName());
}
