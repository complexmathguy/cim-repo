...
public class DomainVersionQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DomainVersionBusinessDelegate.getDomainVersionInstance()
              .getDomainVersion(new DomainVersionFetchOneSummary(domainVersionId));
    } catch (Throwable exc) {
  ...
  public List<DomainVersion> loadAll() {
    ...
    try {
      ...
      domainVersionList =
          DomainVersionBusinessDelegate.getDomainVersionInstance().getAllDomainVersion();
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DomainVersionQueryRestController.class.getName());
}
