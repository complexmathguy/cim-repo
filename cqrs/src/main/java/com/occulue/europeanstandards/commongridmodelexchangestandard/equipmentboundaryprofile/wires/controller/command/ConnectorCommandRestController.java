...
public class ConnectorCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateConnectorCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateConnectorCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "ConnectorController:update() - successfully update Connector - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID connectorId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted Connector with key " + command.getConnectorId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConnectorCommandRestController.class.getName());
}
