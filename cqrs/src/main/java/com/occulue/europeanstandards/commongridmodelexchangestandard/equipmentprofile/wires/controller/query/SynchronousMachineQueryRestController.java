...
public class SynchronousMachineQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          SynchronousMachineBusinessDelegate.getSynchronousMachineInstance()
              .getSynchronousMachine(new SynchronousMachineFetchOneSummary(synchronousMachineId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load SynchronousMachine using Id " + synchronousMachineId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(SynchronousMachineQueryRestController.class.getName());
}
