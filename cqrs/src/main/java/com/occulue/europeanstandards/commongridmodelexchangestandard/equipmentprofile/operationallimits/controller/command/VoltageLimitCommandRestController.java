...
public class VoltageLimitCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateVoltageLimitCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          VoltageLimitBusinessDelegate.getVoltageLimitInstance().createVoltageLimit(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateVoltageLimitCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          VoltageLimitBusinessDelegate.getVoltageLimitInstance().updateVoltageLimit(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "VoltageLimitController:update() - successfully update VoltageLimit - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID voltageLimitId) {
    ...
    try {
      VoltageLimitBusinessDelegate delegate =
          VoltageLimitBusinessDelegate.getVoltageLimitInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted VoltageLimit with key " + command.getVoltageLimitId());
    } catch (Throwable exc) {
  ...
  public void unAssignValue(
      @RequestBody(required = true) UnAssignValueFromVoltageLimitCommand command) {
    try {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(VoltageLimitCommandRestController.class.getName());
}
