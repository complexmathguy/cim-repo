...
public class GroundDisconnectorCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateGroundDisconnectorCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          GroundDisconnectorBusinessDelegate.getGroundDisconnectorInstance()
              .createGroundDisconnector(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateGroundDisconnectorCommand command) {
    ...
    try {
      ...
      completableFuture =
          GroundDisconnectorBusinessDelegate.getGroundDisconnectorInstance()
              .updateGroundDisconnector(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "GroundDisconnectorController:update() - successfully update GroundDisconnector - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID groundDisconnectorId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteGroundDisconnectorCommand command =
        new DeleteGroundDisconnectorCommand(groundDisconnectorId);

    try {
      GroundDisconnectorBusinessDelegate delegate =
          GroundDisconnectorBusinessDelegate.getGroundDisconnectorInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted GroundDisconnector with key " + command.getGroundDisconnectorId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GroundDisconnectorCommandRestController.class.getName());
}
