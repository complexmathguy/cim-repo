...
public class DCChopperCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDCChopperCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDCChopperCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "DCChopperController:update() - successfully update DCChopper - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID dCChopperId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted DCChopper with key " + command.getDCChopperId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCChopperCommandRestController.class.getName());
}
