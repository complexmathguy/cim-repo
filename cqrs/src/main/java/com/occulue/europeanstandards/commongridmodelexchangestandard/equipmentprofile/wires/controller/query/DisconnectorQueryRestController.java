...
public class DisconnectorQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DisconnectorBusinessDelegate.getDisconnectorInstance()
              .getDisconnector(new DisconnectorFetchOneSummary(disconnectorId));
    } catch (Throwable exc) {
  ...
  public List<Disconnector> loadAll() {
    ...
    try {
      ...
      disconnectorList =
          DisconnectorBusinessDelegate.getDisconnectorInstance().getAllDisconnector();

      if (disconnectorList != null) LOGGER.log(Level.INFO, "successfully loaded all Disconnectors");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DisconnectorQueryRestController.class.getName());
}
