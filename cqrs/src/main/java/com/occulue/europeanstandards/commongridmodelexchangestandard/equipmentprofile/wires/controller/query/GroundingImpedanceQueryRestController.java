...
public class GroundingImpedanceQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GroundingImpedanceBusinessDelegate.getGroundingImpedanceInstance()
              .getGroundingImpedance(new GroundingImpedanceFetchOneSummary(groundingImpedanceId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load GroundingImpedance using Id " + groundingImpedanceId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GroundingImpedanceQueryRestController.class.getName());
}
