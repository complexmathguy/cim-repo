...
public class DecimalProxyCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDecimalProxyCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          DecimalProxyBusinessDelegate.getDecimalProxyInstance().createDecimalProxy(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDecimalProxyCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          DecimalProxyBusinessDelegate.getDecimalProxyInstance().updateDecimalProxy(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "DecimalProxyController:update() - successfully update DecimalProxy - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID decimalProxyId) {
    ...
    try {
      DecimalProxyBusinessDelegate delegate =
          DecimalProxyBusinessDelegate.getDecimalProxyInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted DecimalProxy with key " + command.getDecimalProxyId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DecimalProxyCommandRestController.class.getName());
}
