...
public class NonConformLoadScheduleQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          NonConformLoadScheduleBusinessDelegate.getNonConformLoadScheduleInstance()
              .getNonConformLoadSchedule(
                  new NonConformLoadScheduleFetchOneSummary(nonConformLoadScheduleId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load NonConformLoadSchedule using Id " + nonConformLoadScheduleId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(NonConformLoadScheduleQueryRestController.class.getName());
}
