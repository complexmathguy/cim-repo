...
public class PssPTIST1QueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          PssPTIST1BusinessDelegate.getPssPTIST1Instance()
              .getPssPTIST1(new PssPTIST1FetchOneSummary(pssPTIST1Id));
    } catch (Throwable exc) {
  ...
  public List<PssPTIST1> loadAll() {
    ...
    try {
      ...
      if (pssPTIST1List != null) LOGGER.log(Level.INFO, "successfully loaded all PssPTIST1s");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PssPTIST1QueryRestController.class.getName());
}
