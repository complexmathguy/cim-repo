...
public class ActivePowerQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ActivePowerBusinessDelegate.getActivePowerInstance()
              .getActivePower(new ActivePowerFetchOneSummary(activePowerId));
    } catch (Throwable exc) {
  ...
  public List<ActivePower> loadAll() {
    ...
    try {
      ...
      if (activePowerList != null) LOGGER.log(Level.INFO, "successfully loaded all ActivePowers");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ActivePowerQueryRestController.class.getName());
}
