...
public class SeasonQueryRestController extends BaseSpringRestController {
  ...
  public Season load(@RequestParam(required = true) UUID seasonId) {
    ...
    try {
      entity =
          SeasonBusinessDelegate.getSeasonInstance().getSeason(new SeasonFetchOneSummary(seasonId));
    } catch (Throwable exc) {
  ...
  public List<Season> loadAll() {
    ...
    try {
      ...
      if (seasonList != null) LOGGER.log(Level.INFO, "successfully loaded all Seasons");
    } catch (Throwable exc) {
