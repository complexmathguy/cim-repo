...
public class AngleRadiansQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          AngleRadiansBusinessDelegate.getAngleRadiansInstance()
              .getAngleRadians(new AngleRadiansFetchOneSummary(angleRadiansId));
    } catch (Throwable exc) {
  ...
  public List<AngleRadians> loadAll() {
    ...
    try {
      ...
      angleRadiansList =
          AngleRadiansBusinessDelegate.getAngleRadiansInstance().getAllAngleRadians();

      if (angleRadiansList != null) LOGGER.log(Level.INFO, "successfully loaded all AngleRadianss");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(AngleRadiansQueryRestController.class.getName());
}
