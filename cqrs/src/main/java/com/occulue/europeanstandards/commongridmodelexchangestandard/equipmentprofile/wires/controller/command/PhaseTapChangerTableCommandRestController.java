...
public class PhaseTapChangerTableCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreatePhaseTapChangerTableCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance()
              .createPhaseTapChangerTable(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdatePhaseTapChangerTableCommand command) {
    ...
    try {
      ...
      completableFuture =
          PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance()
              .updatePhaseTapChangerTable(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "PhaseTapChangerTableController:update() - successfully update PhaseTapChangerTable - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(
      @RequestParam(required = true) UUID phaseTapChangerTableId) {
    CompletableFuture<Void> completableFuture = null;
    DeletePhaseTapChangerTableCommand command =
        new DeletePhaseTapChangerTableCommand(phaseTapChangerTableId);

    try {
      PhaseTapChangerTableBusinessDelegate delegate =
          PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted PhaseTapChangerTable with key "
              + command.getPhaseTapChangerTableId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerTableCommandRestController.class.getName());
}
