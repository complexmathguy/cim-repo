...
public class GovSteam0QueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovSteam0BusinessDelegate.getGovSteam0Instance()
              .getGovSteam0(new GovSteam0FetchOneSummary(govSteam0Id));
    } catch (Throwable exc) {
  ...
  public List<GovSteam0> loadAll() {
    ...
    try {
      ...
      if (govSteam0List != null) LOGGER.log(Level.INFO, "successfully loaded all GovSteam0s");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovSteam0QueryRestController.class.getName());
}
