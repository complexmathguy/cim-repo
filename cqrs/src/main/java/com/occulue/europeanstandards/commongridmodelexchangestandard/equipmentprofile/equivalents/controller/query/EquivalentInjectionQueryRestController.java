...
public class EquivalentInjectionQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          EquivalentInjectionBusinessDelegate.getEquivalentInjectionInstance()
              .getEquivalentInjection(
                  new EquivalentInjectionFetchOneSummary(equivalentInjectionId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load EquivalentInjection using Id " + equivalentInjectionId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EquivalentInjectionQueryRestController.class.getName());
}
