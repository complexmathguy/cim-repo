...
public class GeographicalRegionCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateGeographicalRegionCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          GeographicalRegionBusinessDelegate.getGeographicalRegionInstance()
              .createGeographicalRegion(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateGeographicalRegionCommand command) {
    ...
    try {
      ...
      completableFuture =
          GeographicalRegionBusinessDelegate.getGeographicalRegionInstance()
              .updateGeographicalRegion(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "GeographicalRegionController:update() - successfully update GeographicalRegion - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID geographicalRegionId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteGeographicalRegionCommand command =
        new DeleteGeographicalRegionCommand(geographicalRegionId);

    try {
      GeographicalRegionBusinessDelegate delegate =
          GeographicalRegionBusinessDelegate.getGeographicalRegionInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted GeographicalRegion with key " + command.getGeographicalRegionId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GeographicalRegionCommandRestController.class.getName());
}
