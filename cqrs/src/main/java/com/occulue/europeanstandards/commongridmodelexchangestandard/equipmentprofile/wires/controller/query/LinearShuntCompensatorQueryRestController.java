...
public class LinearShuntCompensatorQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          LinearShuntCompensatorBusinessDelegate.getLinearShuntCompensatorInstance()
              .getLinearShuntCompensator(
                  new LinearShuntCompensatorFetchOneSummary(linearShuntCompensatorId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load LinearShuntCompensator using Id " + linearShuntCompensatorId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(LinearShuntCompensatorQueryRestController.class.getName());
}
