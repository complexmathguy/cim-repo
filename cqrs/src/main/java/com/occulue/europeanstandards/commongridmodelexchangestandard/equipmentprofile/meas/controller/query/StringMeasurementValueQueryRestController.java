...
public class StringMeasurementValueQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          StringMeasurementValueBusinessDelegate.getStringMeasurementValueInstance()
              .getStringMeasurementValue(
                  new StringMeasurementValueFetchOneSummary(stringMeasurementValueId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load StringMeasurementValue using Id " + stringMeasurementValueId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(StringMeasurementValueQueryRestController.class.getName());
}
