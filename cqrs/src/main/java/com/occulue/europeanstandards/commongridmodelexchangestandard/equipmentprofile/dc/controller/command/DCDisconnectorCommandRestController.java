...
public class DCDisconnectorCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDCDisconnectorCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          DCDisconnectorBusinessDelegate.getDCDisconnectorInstance().createDCDisconnector(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDCDisconnectorCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          DCDisconnectorBusinessDelegate.getDCDisconnectorInstance().updateDCDisconnector(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "DCDisconnectorController:update() - successfully update DCDisconnector - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID dCDisconnectorId) {
    ...
    try {
      DCDisconnectorBusinessDelegate delegate =
          DCDisconnectorBusinessDelegate.getDCDisconnectorInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted DCDisconnector with key " + command.getDCDisconnectorId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCDisconnectorCommandRestController.class.getName());
}
