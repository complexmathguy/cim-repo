...
public class LengthQueryRestController extends BaseSpringRestController {
  ...
  public Length load(@RequestParam(required = true) UUID lengthId) {
    ...
    try {
      entity =
          LengthBusinessDelegate.getLengthInstance().getLength(new LengthFetchOneSummary(lengthId));
    } catch (Throwable exc) {
  ...
  public List<Length> loadAll() {
    ...
    try {
      ...
      if (lengthList != null) LOGGER.log(Level.INFO, "successfully loaded all Lengths");
    } catch (Throwable exc) {
