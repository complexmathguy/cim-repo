...
public class DiscreteValueQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DiscreteValueBusinessDelegate.getDiscreteValueInstance()
              .getDiscreteValue(new DiscreteValueFetchOneSummary(discreteValueId));
    } catch (Throwable exc) {
  ...
  public List<DiscreteValue> loadAll() {
    ...
    try {
      ...
      discreteValueList =
          DiscreteValueBusinessDelegate.getDiscreteValueInstance().getAllDiscreteValue();
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiscreteValueQueryRestController.class.getName());
}
