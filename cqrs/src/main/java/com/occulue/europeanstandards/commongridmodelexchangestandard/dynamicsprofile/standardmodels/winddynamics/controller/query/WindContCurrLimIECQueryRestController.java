...
public class WindContCurrLimIECQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          WindContCurrLimIECBusinessDelegate.getWindContCurrLimIECInstance()
              .getWindContCurrLimIEC(new WindContCurrLimIECFetchOneSummary(windContCurrLimIECId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load WindContCurrLimIEC using Id " + windContCurrLimIECId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(WindContCurrLimIECQueryRestController.class.getName());
}
