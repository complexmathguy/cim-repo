...
public class FossilFuelQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          FossilFuelBusinessDelegate.getFossilFuelInstance()
              .getFossilFuel(new FossilFuelFetchOneSummary(fossilFuelId));
    } catch (Throwable exc) {
  ...
  public List<FossilFuel> loadAll() {
    ...
    try {
      ...
      if (fossilFuelList != null) LOGGER.log(Level.INFO, "successfully loaded all FossilFuels");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(FossilFuelQueryRestController.class.getName());
}
