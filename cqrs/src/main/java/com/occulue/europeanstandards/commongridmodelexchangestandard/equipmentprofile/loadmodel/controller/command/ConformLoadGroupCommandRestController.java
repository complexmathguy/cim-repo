...
public class ConformLoadGroupCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateConformLoadGroupCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          ConformLoadGroupBusinessDelegate.getConformLoadGroupInstance()
              .createConformLoadGroup(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateConformLoadGroupCommand command) {
    ...
    try {
      ...
      completableFuture =
          ConformLoadGroupBusinessDelegate.getConformLoadGroupInstance()
              .updateConformLoadGroup(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "ConformLoadGroupController:update() - successfully update ConformLoadGroup - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID conformLoadGroupId) {
    ...
    try {
      ConformLoadGroupBusinessDelegate delegate =
          ConformLoadGroupBusinessDelegate.getConformLoadGroupInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted ConformLoadGroup with key " + command.getConformLoadGroupId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConformLoadGroupCommandRestController.class.getName());
}
