...
public class VoltageLimitQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          VoltageLimitBusinessDelegate.getVoltageLimitInstance()
              .getVoltageLimit(new VoltageLimitFetchOneSummary(voltageLimitId));
    } catch (Throwable exc) {
  ...
  public List<VoltageLimit> loadAll() {
    ...
    try {
      ...
      voltageLimitList =
          VoltageLimitBusinessDelegate.getVoltageLimitInstance().getAllVoltageLimit();

      if (voltageLimitList != null) LOGGER.log(Level.INFO, "successfully loaded all VoltageLimits");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(VoltageLimitQueryRestController.class.getName());
}
