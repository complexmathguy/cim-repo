...
public class SolarGeneratingUnitCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateSolarGeneratingUnitCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          SolarGeneratingUnitBusinessDelegate.getSolarGeneratingUnitInstance()
              .createSolarGeneratingUnit(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateSolarGeneratingUnitCommand command) {
    ...
    try {
      ...
      completableFuture =
          SolarGeneratingUnitBusinessDelegate.getSolarGeneratingUnitInstance()
              .updateSolarGeneratingUnit(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "SolarGeneratingUnitController:update() - successfully update SolarGeneratingUnit - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID solarGeneratingUnitId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteSolarGeneratingUnitCommand command =
        new DeleteSolarGeneratingUnitCommand(solarGeneratingUnitId);

    try {
      SolarGeneratingUnitBusinessDelegate delegate =
          SolarGeneratingUnitBusinessDelegate.getSolarGeneratingUnitInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted SolarGeneratingUnit with key "
              + command.getSolarGeneratingUnitId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(SolarGeneratingUnitCommandRestController.class.getName());
}
