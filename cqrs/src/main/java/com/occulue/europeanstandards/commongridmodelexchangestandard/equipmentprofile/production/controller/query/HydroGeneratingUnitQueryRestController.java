...
public class HydroGeneratingUnitQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          HydroGeneratingUnitBusinessDelegate.getHydroGeneratingUnitInstance()
              .getHydroGeneratingUnit(
                  new HydroGeneratingUnitFetchOneSummary(hydroGeneratingUnitId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load HydroGeneratingUnit using Id " + hydroGeneratingUnitId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(HydroGeneratingUnitQueryRestController.class.getName());
}
