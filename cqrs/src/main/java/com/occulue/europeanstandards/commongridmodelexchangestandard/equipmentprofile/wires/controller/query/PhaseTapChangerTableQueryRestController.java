...
public class PhaseTapChangerTableQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance()
              .getPhaseTapChangerTable(
                  new PhaseTapChangerTableFetchOneSummary(phaseTapChangerTableId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load PhaseTapChangerTable using Id " + phaseTapChangerTableId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerTableQueryRestController.class.getName());
}
