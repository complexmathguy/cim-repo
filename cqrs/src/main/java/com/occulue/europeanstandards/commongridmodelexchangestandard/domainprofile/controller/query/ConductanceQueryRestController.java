...
public class ConductanceQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ConductanceBusinessDelegate.getConductanceInstance()
              .getConductance(new ConductanceFetchOneSummary(conductanceId));
    } catch (Throwable exc) {
  ...
  public List<Conductance> loadAll() {
    ...
    try {
      ...
      if (conductanceList != null) LOGGER.log(Level.INFO, "successfully loaded all Conductances");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConductanceQueryRestController.class.getName());
}
