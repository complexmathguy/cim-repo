...
public class DayTypeCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDayTypeCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDayTypeCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "DayTypeController:update() - successfully update DayType - " + exc.getMessage());
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DayTypeCommandRestController.class.getName());
}
