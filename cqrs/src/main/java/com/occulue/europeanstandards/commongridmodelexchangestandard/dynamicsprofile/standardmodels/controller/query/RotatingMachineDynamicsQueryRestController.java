...
public class RotatingMachineDynamicsQueryRestController extends BaseSpringRestController {
   ...
   */
  @GetMapping("/load")
  public RotatingMachineDynamics load(
    ...
    try {
      entity =
          RotatingMachineDynamicsBusinessDelegate.getRotatingMachineDynamicsInstance()
              .getRotatingMachineDynamics(
                  new RotatingMachineDynamicsFetchOneSummary(rotatingMachineDynamicsId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load RotatingMachineDynamics using Id " + rotatingMachineDynamicsId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(RotatingMachineDynamicsQueryRestController.class.getName());
}
