...
public class DiagramStyleQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DiagramStyleBusinessDelegate.getDiagramStyleInstance()
              .getDiagramStyle(new DiagramStyleFetchOneSummary(diagramStyleId));
    } catch (Throwable exc) {
  ...
  public List<DiagramStyle> loadAll() {
    ...
    try {
      ...
      diagramStyleList =
          DiagramStyleBusinessDelegate.getDiagramStyleInstance().getAllDiagramStyle();

      if (diagramStyleList != null) LOGGER.log(Level.INFO, "successfully loaded all DiagramStyles");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramStyleQueryRestController.class.getName());
}
