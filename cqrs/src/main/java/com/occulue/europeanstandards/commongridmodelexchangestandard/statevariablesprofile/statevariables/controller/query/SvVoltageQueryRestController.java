...
public class SvVoltageQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          SvVoltageBusinessDelegate.getSvVoltageInstance()
              .getSvVoltage(new SvVoltageFetchOneSummary(svVoltageId));
    } catch (Throwable exc) {
  ...
  public List<SvVoltage> loadAll() {
    ...
    try {
      ...
      if (svVoltageList != null) LOGGER.log(Level.INFO, "successfully loaded all SvVoltages");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(SvVoltageQueryRestController.class.getName());
}
