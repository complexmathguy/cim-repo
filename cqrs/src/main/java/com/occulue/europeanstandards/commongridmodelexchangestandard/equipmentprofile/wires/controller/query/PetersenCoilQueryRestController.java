...
public class PetersenCoilQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          PetersenCoilBusinessDelegate.getPetersenCoilInstance()
              .getPetersenCoil(new PetersenCoilFetchOneSummary(petersenCoilId));
    } catch (Throwable exc) {
  ...
  public List<PetersenCoil> loadAll() {
    ...
    try {
      ...
      petersenCoilList =
          PetersenCoilBusinessDelegate.getPetersenCoilInstance().getAllPetersenCoil();

      if (petersenCoilList != null) LOGGER.log(Level.INFO, "successfully loaded all PetersenCoils");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PetersenCoilQueryRestController.class.getName());
}
