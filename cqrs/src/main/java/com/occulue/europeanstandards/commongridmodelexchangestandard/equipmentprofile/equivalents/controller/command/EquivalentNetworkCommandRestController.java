...
public class EquivalentNetworkCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateEquivalentNetworkCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          EquivalentNetworkBusinessDelegate.getEquivalentNetworkInstance()
              .createEquivalentNetwork(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateEquivalentNetworkCommand command) {
    ...
    try {
      ...
      completableFuture =
          EquivalentNetworkBusinessDelegate.getEquivalentNetworkInstance()
              .updateEquivalentNetwork(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "EquivalentNetworkController:update() - successfully update EquivalentNetwork - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID equivalentNetworkId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteEquivalentNetworkCommand command =
        new DeleteEquivalentNetworkCommand(equivalentNetworkId);

    try {
      EquivalentNetworkBusinessDelegate delegate =
          EquivalentNetworkBusinessDelegate.getEquivalentNetworkInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted EquivalentNetwork with key " + command.getEquivalentNetworkId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EquivalentNetworkCommandRestController.class.getName());
}
