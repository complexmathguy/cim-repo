...
public class ExternalNetworkInjectionQueryRestController extends BaseSpringRestController {
   ...
   */
  @GetMapping("/load")
  public ExternalNetworkInjection load(
    ...
    try {
      entity =
          ExternalNetworkInjectionBusinessDelegate.getExternalNetworkInjectionInstance()
              .getExternalNetworkInjection(
                  new ExternalNetworkInjectionFetchOneSummary(externalNetworkInjectionId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load ExternalNetworkInjection using Id " + externalNetworkInjectionId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ExternalNetworkInjectionQueryRestController.class.getName());
}
