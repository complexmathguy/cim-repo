...
public class DecimalProxyQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DecimalProxyBusinessDelegate.getDecimalProxyInstance()
              .getDecimalProxy(new DecimalProxyFetchOneSummary(decimalProxyId));
    } catch (Throwable exc) {
  ...
  public List<DecimalProxy> loadAll() {
    ...
    try {
      ...
      decimalProxyList =
          DecimalProxyBusinessDelegate.getDecimalProxyInstance().getAllDecimalProxy();

      if (decimalProxyList != null) LOGGER.log(Level.INFO, "successfully loaded all DecimalProxys");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DecimalProxyQueryRestController.class.getName());
}
