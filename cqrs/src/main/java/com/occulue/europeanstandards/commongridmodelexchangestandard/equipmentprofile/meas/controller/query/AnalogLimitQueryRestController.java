...
public class AnalogLimitQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          AnalogLimitBusinessDelegate.getAnalogLimitInstance()
              .getAnalogLimit(new AnalogLimitFetchOneSummary(analogLimitId));
    } catch (Throwable exc) {
  ...
  public List<AnalogLimit> loadAll() {
    ...
    try {
      ...
      if (analogLimitList != null) LOGGER.log(Level.INFO, "successfully loaded all AnalogLimits");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(AnalogLimitQueryRestController.class.getName());
}
