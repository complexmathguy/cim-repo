...
public class DateTimeQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DateTimeBusinessDelegate.getDateTimeInstance()
              .getDateTime(new DateTimeFetchOneSummary(dateTimeId));
    } catch (Throwable exc) {
  ...
  public List<DateTime> loadAll() {
    ...
    try {
      ...
      if (dateTimeList != null) LOGGER.log(Level.INFO, "successfully loaded all DateTimes");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DateTimeQueryRestController.class.getName());
}
