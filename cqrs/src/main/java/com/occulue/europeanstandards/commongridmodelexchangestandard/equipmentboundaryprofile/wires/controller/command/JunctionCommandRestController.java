...
public class JunctionCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateJunctionCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateJunctionCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "JunctionController:update() - successfully update Junction - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID junctionId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted Junction with key " + command.getJunctionId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(JunctionCommandRestController.class.getName());
}
