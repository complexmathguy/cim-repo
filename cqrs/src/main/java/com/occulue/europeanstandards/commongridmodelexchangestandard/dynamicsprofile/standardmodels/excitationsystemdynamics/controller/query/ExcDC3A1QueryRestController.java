...
public class ExcDC3A1QueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ExcDC3A1BusinessDelegate.getExcDC3A1Instance()
              .getExcDC3A1(new ExcDC3A1FetchOneSummary(excDC3A1Id));
    } catch (Throwable exc) {
  ...
  public List<ExcDC3A1> loadAll() {
    ...
    try {
      ...
      if (excDC3A1List != null) LOGGER.log(Level.INFO, "successfully loaded all ExcDC3A1s");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ExcDC3A1QueryRestController.class.getName());
}
