...
public class Pss2STQueryRestController extends BaseSpringRestController {
  ...
  public Pss2ST load(@RequestParam(required = true) UUID pss2STId) {
    ...
    try {
      entity =
          Pss2STBusinessDelegate.getPss2STInstance().getPss2ST(new Pss2STFetchOneSummary(pss2STId));
    } catch (Throwable exc) {
  ...
  public List<Pss2ST> loadAll() {
    ...
    try {
      ...
      if (pss2STList != null) LOGGER.log(Level.INFO, "successfully loaded all Pss2STs");
    } catch (Throwable exc) {
