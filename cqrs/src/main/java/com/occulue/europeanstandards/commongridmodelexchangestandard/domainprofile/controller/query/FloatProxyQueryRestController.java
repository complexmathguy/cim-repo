...
public class FloatProxyQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          FloatProxyBusinessDelegate.getFloatProxyInstance()
              .getFloatProxy(new FloatProxyFetchOneSummary(floatProxyId));
    } catch (Throwable exc) {
  ...
  public List<FloatProxy> loadAll() {
    ...
    try {
      ...
      if (floatProxyList != null) LOGGER.log(Level.INFO, "successfully loaded all FloatProxys");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(FloatProxyQueryRestController.class.getName());
}
