...
public class DCBaseTerminalCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDCBaseTerminalCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          DCBaseTerminalBusinessDelegate.getDCBaseTerminalInstance().createDCBaseTerminal(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDCBaseTerminalCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          DCBaseTerminalBusinessDelegate.getDCBaseTerminalInstance().updateDCBaseTerminal(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "DCBaseTerminalController:update() - successfully update DCBaseTerminal - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID dCBaseTerminalId) {
    ...
    try {
      DCBaseTerminalBusinessDelegate delegate =
          DCBaseTerminalBusinessDelegate.getDCBaseTerminalInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted DCBaseTerminal with key " + command.getDCBaseTerminalId());
    } catch (Throwable exc) {
  ...
  /**
   * save DCTerminals on DCBaseTerminal
   *
   * @param command AssignDCTerminalsToDCBaseTerminalCommand
   */
  @PutMapping("/addToDCTerminals")
  public void addToDCTerminals(
      @RequestBody(required = true) AssignDCTerminalsToDCBaseTerminalCommand command) {
    try {
      DCBaseTerminalBusinessDelegate.getDCBaseTerminalInstance().addToDCTerminals(command);
    } catch (Exception exc) {
  ...
  /**
   * remove DCTerminals on DCBaseTerminal
   *
  ...
  public void removeFromDCTerminals(
      @RequestBody(required = true) RemoveDCTerminalsFromDCBaseTerminalCommand command) {
    try {
      DCBaseTerminalBusinessDelegate.getDCBaseTerminalInstance().removeFromDCTerminals(command);
    } catch (Exception exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCBaseTerminalCommandRestController.class.getName());
}
