...
public class ValueAliasSetCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateValueAliasSetCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          ValueAliasSetBusinessDelegate.getValueAliasSetInstance().createValueAliasSet(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateValueAliasSetCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          ValueAliasSetBusinessDelegate.getValueAliasSetInstance().updateValueAliasSet(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "ValueAliasSetController:update() - successfully update ValueAliasSet - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID valueAliasSetId) {
    ...
    try {
      ValueAliasSetBusinessDelegate delegate =
          ValueAliasSetBusinessDelegate.getValueAliasSetInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted ValueAliasSet with key " + command.getValueAliasSetId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ValueAliasSetCommandRestController.class.getName());
}
