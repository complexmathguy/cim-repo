...
public class DCBusbarCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDCBusbarCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDCBusbarCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "DCBusbarController:update() - successfully update DCBusbar - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID dCBusbarId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted DCBusbar with key " + command.getDCBusbarId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCBusbarCommandRestController.class.getName());
}
