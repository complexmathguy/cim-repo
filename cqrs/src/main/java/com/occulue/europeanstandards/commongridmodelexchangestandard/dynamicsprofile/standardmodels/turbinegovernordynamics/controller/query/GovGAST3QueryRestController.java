...
public class GovGAST3QueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovGAST3BusinessDelegate.getGovGAST3Instance()
              .getGovGAST3(new GovGAST3FetchOneSummary(govGAST3Id));
    } catch (Throwable exc) {
  ...
  public List<GovGAST3> loadAll() {
    ...
    try {
      ...
      if (govGAST3List != null) LOGGER.log(Level.INFO, "successfully loaded all GovGAST3s");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovGAST3QueryRestController.class.getName());
}
