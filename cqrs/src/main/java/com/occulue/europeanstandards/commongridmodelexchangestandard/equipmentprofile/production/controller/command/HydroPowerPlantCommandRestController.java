...
public class HydroPowerPlantCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateHydroPowerPlantCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          HydroPowerPlantBusinessDelegate.getHydroPowerPlantInstance()
              .createHydroPowerPlant(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateHydroPowerPlantCommand command) {
    ...
    try {
      ...
      completableFuture =
          HydroPowerPlantBusinessDelegate.getHydroPowerPlantInstance()
              .updateHydroPowerPlant(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "HydroPowerPlantController:update() - successfully update HydroPowerPlant - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID hydroPowerPlantId) {
    ...
    try {
      HydroPowerPlantBusinessDelegate delegate =
          HydroPowerPlantBusinessDelegate.getHydroPowerPlantInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted HydroPowerPlant with key " + command.getHydroPowerPlantId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(HydroPowerPlantCommandRestController.class.getName());
}
