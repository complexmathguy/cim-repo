...
public class EarthFaultCompensatorQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          EarthFaultCompensatorBusinessDelegate.getEarthFaultCompensatorInstance()
              .getEarthFaultCompensator(
                  new EarthFaultCompensatorFetchOneSummary(earthFaultCompensatorId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load EarthFaultCompensator using Id " + earthFaultCompensatorId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EarthFaultCompensatorQueryRestController.class.getName());
}
