...
public class AccumulatorCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateAccumulatorCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          AccumulatorBusinessDelegate.getAccumulatorInstance().createAccumulator(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateAccumulatorCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          AccumulatorBusinessDelegate.getAccumulatorInstance().updateAccumulator(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "AccumulatorController:update() - successfully update Accumulator - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID accumulatorId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted Accumulator with key " + command.getAccumulatorId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(AccumulatorCommandRestController.class.getName());
}
