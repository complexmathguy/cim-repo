...
public class DisconnectorCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDisconnectorCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          DisconnectorBusinessDelegate.getDisconnectorInstance().createDisconnector(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDisconnectorCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          DisconnectorBusinessDelegate.getDisconnectorInstance().updateDisconnector(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "DisconnectorController:update() - successfully update Disconnector - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID disconnectorId) {
    ...
    try {
      DisconnectorBusinessDelegate delegate =
          DisconnectorBusinessDelegate.getDisconnectorInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted Disconnector with key " + command.getDisconnectorId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DisconnectorCommandRestController.class.getName());
}
