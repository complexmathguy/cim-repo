...
public class LoadDynamicsCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateLoadDynamicsCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          LoadDynamicsBusinessDelegate.getLoadDynamicsInstance().createLoadDynamics(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateLoadDynamicsCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          LoadDynamicsBusinessDelegate.getLoadDynamicsInstance().updateLoadDynamics(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "LoadDynamicsController:update() - successfully update LoadDynamics - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID loadDynamicsId) {
    ...
    try {
      LoadDynamicsBusinessDelegate delegate =
          LoadDynamicsBusinessDelegate.getLoadDynamicsInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted LoadDynamics with key " + command.getLoadDynamicsId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(LoadDynamicsCommandRestController.class.getName());
}
