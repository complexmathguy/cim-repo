...
public class PhaseTapChangerTablePointQueryRestController extends BaseSpringRestController {
   ...
   */
  @GetMapping("/load")
  public PhaseTapChangerTablePoint load(
    ...
    try {
      entity =
          PhaseTapChangerTablePointBusinessDelegate.getPhaseTapChangerTablePointInstance()
              .getPhaseTapChangerTablePoint(
                  new PhaseTapChangerTablePointFetchOneSummary(phaseTapChangerTablePointId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load PhaseTapChangerTablePoint using Id " + phaseTapChangerTablePointId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerTablePointQueryRestController.class.getName());
}
