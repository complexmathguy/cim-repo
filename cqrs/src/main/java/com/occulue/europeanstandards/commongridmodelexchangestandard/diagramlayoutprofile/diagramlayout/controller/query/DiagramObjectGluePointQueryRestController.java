...
public class DiagramObjectGluePointQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DiagramObjectGluePointBusinessDelegate.getDiagramObjectGluePointInstance()
              .getDiagramObjectGluePoint(
                  new DiagramObjectGluePointFetchOneSummary(diagramObjectGluePointId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load DiagramObjectGluePoint using Id " + diagramObjectGluePointId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramObjectGluePointQueryRestController.class.getName());
}
