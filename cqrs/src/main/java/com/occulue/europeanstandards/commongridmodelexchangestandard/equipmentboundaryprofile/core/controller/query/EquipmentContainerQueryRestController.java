...
public class EquipmentContainerQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          EquipmentContainerBusinessDelegate.getEquipmentContainerInstance()
              .getEquipmentContainer(new EquipmentContainerFetchOneSummary(equipmentContainerId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load EquipmentContainer using Id " + equipmentContainerId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EquipmentContainerQueryRestController.class.getName());
}
