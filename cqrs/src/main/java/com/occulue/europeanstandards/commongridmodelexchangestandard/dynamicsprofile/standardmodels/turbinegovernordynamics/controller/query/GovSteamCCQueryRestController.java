...
public class GovSteamCCQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovSteamCCBusinessDelegate.getGovSteamCCInstance()
              .getGovSteamCC(new GovSteamCCFetchOneSummary(govSteamCCId));
    } catch (Throwable exc) {
  ...
  public List<GovSteamCC> loadAll() {
    ...
    try {
      ...
      if (govSteamCCList != null) LOGGER.log(Level.INFO, "successfully loaded all GovSteamCCs");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovSteamCCQueryRestController.class.getName());
}
