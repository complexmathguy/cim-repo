...
public class WindAeroConstIECCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateWindAeroConstIECCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          WindAeroConstIECBusinessDelegate.getWindAeroConstIECInstance()
              .createWindAeroConstIEC(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateWindAeroConstIECCommand command) {
    ...
    try {
      ...
      completableFuture =
          WindAeroConstIECBusinessDelegate.getWindAeroConstIECInstance()
              .updateWindAeroConstIEC(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "WindAeroConstIECController:update() - successfully update WindAeroConstIEC - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID windAeroConstIECId) {
    ...
    try {
      WindAeroConstIECBusinessDelegate delegate =
          WindAeroConstIECBusinessDelegate.getWindAeroConstIECInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted WindAeroConstIEC with key " + command.getWindAeroConstIECId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(WindAeroConstIECCommandRestController.class.getName());
}
