...
public class DCSwitchCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDCSwitchCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDCSwitchCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "DCSwitchController:update() - successfully update DCSwitch - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID dCSwitchId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted DCSwitch with key " + command.getDCSwitchId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCSwitchCommandRestController.class.getName());
}
