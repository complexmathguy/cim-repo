...
public class TapChangerControlCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateTapChangerControlCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          TapChangerControlBusinessDelegate.getTapChangerControlInstance()
              .createTapChangerControl(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateTapChangerControlCommand command) {
    ...
    try {
      ...
      completableFuture =
          TapChangerControlBusinessDelegate.getTapChangerControlInstance()
              .updateTapChangerControl(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "TapChangerControlController:update() - successfully update TapChangerControl - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID tapChangerControlId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteTapChangerControlCommand command =
        new DeleteTapChangerControlCommand(tapChangerControlId);

    try {
      TapChangerControlBusinessDelegate delegate =
          TapChangerControlBusinessDelegate.getTapChangerControlInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted TapChangerControl with key " + command.getTapChangerControlId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(TapChangerControlCommandRestController.class.getName());
}
