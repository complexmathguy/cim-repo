...
public class ResistancePerLengthQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ResistancePerLengthBusinessDelegate.getResistancePerLengthInstance()
              .getResistancePerLength(
                  new ResistancePerLengthFetchOneSummary(resistancePerLengthId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load ResistancePerLength using Id " + resistancePerLengthId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ResistancePerLengthQueryRestController.class.getName());
}
