...
public class SubstationQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          SubstationBusinessDelegate.getSubstationInstance()
              .getSubstation(new SubstationFetchOneSummary(substationId));
    } catch (Throwable exc) {
  ...
  public List<Substation> loadAll() {
    ...
    try {
      ...
      if (substationList != null) LOGGER.log(Level.INFO, "successfully loaded all Substations");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(SubstationQueryRestController.class.getName());
}
