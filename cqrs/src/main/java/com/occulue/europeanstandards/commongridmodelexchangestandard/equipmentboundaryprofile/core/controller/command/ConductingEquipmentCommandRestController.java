...
public class ConductingEquipmentCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateConductingEquipmentCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          ConductingEquipmentBusinessDelegate.getConductingEquipmentInstance()
              .createConductingEquipment(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateConductingEquipmentCommand command) {
    ...
    try {
      ...
      completableFuture =
          ConductingEquipmentBusinessDelegate.getConductingEquipmentInstance()
              .updateConductingEquipment(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "ConductingEquipmentController:update() - successfully update ConductingEquipment - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID conductingEquipmentId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteConductingEquipmentCommand command =
        new DeleteConductingEquipmentCommand(conductingEquipmentId);

    try {
      ConductingEquipmentBusinessDelegate delegate =
          ConductingEquipmentBusinessDelegate.getConductingEquipmentInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted ConductingEquipment with key "
              + command.getConductingEquipmentId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConductingEquipmentCommandRestController.class.getName());
}
