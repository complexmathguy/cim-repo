...
public class OperationalLimitTypeQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          OperationalLimitTypeBusinessDelegate.getOperationalLimitTypeInstance()
              .getOperationalLimitType(
                  new OperationalLimitTypeFetchOneSummary(operationalLimitTypeId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load OperationalLimitType using Id " + operationalLimitTypeId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(OperationalLimitTypeQueryRestController.class.getName());
}
