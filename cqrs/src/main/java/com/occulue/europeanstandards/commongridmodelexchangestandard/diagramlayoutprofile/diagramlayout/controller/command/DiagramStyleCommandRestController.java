...
public class DiagramStyleCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDiagramStyleCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          DiagramStyleBusinessDelegate.getDiagramStyleInstance().createDiagramStyle(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDiagramStyleCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          DiagramStyleBusinessDelegate.getDiagramStyleInstance().updateDiagramStyle(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "DiagramStyleController:update() - successfully update DiagramStyle - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID diagramStyleId) {
    ...
    try {
      DiagramStyleBusinessDelegate delegate =
          DiagramStyleBusinessDelegate.getDiagramStyleInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted DiagramStyle with key " + command.getDiagramStyleId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramStyleCommandRestController.class.getName());
}
