...
public class InductancePerLengthQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          InductancePerLengthBusinessDelegate.getInductancePerLengthInstance()
              .getInductancePerLength(
                  new InductancePerLengthFetchOneSummary(inductancePerLengthId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load InductancePerLength using Id " + inductancePerLengthId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(InductancePerLengthQueryRestController.class.getName());
}
