...
public class ValueToAliasQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ValueToAliasBusinessDelegate.getValueToAliasInstance()
              .getValueToAlias(new ValueToAliasFetchOneSummary(valueToAliasId));
    } catch (Throwable exc) {
  ...
  public List<ValueToAlias> loadAll() {
    ...
    try {
      ...
      valueToAliasList =
          ValueToAliasBusinessDelegate.getValueToAliasInstance().getAllValueToAlias();

      if (valueToAliasList != null) LOGGER.log(Level.INFO, "successfully loaded all ValueToAliass");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ValueToAliasQueryRestController.class.getName());
}
