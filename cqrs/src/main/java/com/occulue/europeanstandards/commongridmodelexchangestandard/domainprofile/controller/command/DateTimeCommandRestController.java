...
public class DateTimeCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDateTimeCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDateTimeCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "DateTimeController:update() - successfully update DateTime - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID dateTimeId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted DateTime with key " + command.getDateTimeId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DateTimeCommandRestController.class.getName());
}
