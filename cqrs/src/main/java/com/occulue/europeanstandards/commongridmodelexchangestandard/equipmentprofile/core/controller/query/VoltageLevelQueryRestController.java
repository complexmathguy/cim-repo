...
public class VoltageLevelQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          VoltageLevelBusinessDelegate.getVoltageLevelInstance()
              .getVoltageLevel(new VoltageLevelFetchOneSummary(voltageLevelId));
    } catch (Throwable exc) {
  ...
  public List<VoltageLevel> loadAll() {
    ...
    try {
      ...
      voltageLevelList =
          VoltageLevelBusinessDelegate.getVoltageLevelInstance().getAllVoltageLevel();

      if (voltageLevelList != null) LOGGER.log(Level.INFO, "successfully loaded all VoltageLevels");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(VoltageLevelQueryRestController.class.getName());
}
