...
public class StringMeasurementCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateStringMeasurementCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          StringMeasurementBusinessDelegate.getStringMeasurementInstance()
              .createStringMeasurement(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateStringMeasurementCommand command) {
    ...
    try {
      ...
      completableFuture =
          StringMeasurementBusinessDelegate.getStringMeasurementInstance()
              .updateStringMeasurement(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "StringMeasurementController:update() - successfully update StringMeasurement - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID stringMeasurementId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteStringMeasurementCommand command =
        new DeleteStringMeasurementCommand(stringMeasurementId);

    try {
      StringMeasurementBusinessDelegate delegate =
          StringMeasurementBusinessDelegate.getStringMeasurementInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted StringMeasurement with key " + command.getStringMeasurementId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(StringMeasurementCommandRestController.class.getName());
}
