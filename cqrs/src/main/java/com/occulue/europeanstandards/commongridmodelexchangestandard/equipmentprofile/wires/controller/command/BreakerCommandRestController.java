...
public class BreakerCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateBreakerCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateBreakerCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "BreakerController:update() - successfully update Breaker - " + exc.getMessage());
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(BreakerCommandRestController.class.getName());
}
