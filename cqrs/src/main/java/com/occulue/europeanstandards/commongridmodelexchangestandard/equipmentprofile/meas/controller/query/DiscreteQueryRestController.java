...
public class DiscreteQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DiscreteBusinessDelegate.getDiscreteInstance()
              .getDiscrete(new DiscreteFetchOneSummary(discreteId));
    } catch (Throwable exc) {
  ...
  public List<Discrete> loadAll() {
    ...
    try {
      ...
      if (discreteList != null) LOGGER.log(Level.INFO, "successfully loaded all Discretes");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiscreteQueryRestController.class.getName());
}
