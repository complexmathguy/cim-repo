...
public class DCEquipmentContainerCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateDCEquipmentContainerCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          DCEquipmentContainerBusinessDelegate.getDCEquipmentContainerInstance()
              .createDCEquipmentContainer(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateDCEquipmentContainerCommand command) {
    ...
    try {
      ...
      completableFuture =
          DCEquipmentContainerBusinessDelegate.getDCEquipmentContainerInstance()
              .updateDCEquipmentContainer(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "DCEquipmentContainerController:update() - successfully update DCEquipmentContainer - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(
      @RequestParam(required = true) UUID dCEquipmentContainerId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteDCEquipmentContainerCommand command =
        new DeleteDCEquipmentContainerCommand(dCEquipmentContainerId);

    try {
      DCEquipmentContainerBusinessDelegate delegate =
          DCEquipmentContainerBusinessDelegate.getDCEquipmentContainerInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted DCEquipmentContainer with key "
              + command.getDCEquipmentContainerId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCEquipmentContainerCommandRestController.class.getName());
}
