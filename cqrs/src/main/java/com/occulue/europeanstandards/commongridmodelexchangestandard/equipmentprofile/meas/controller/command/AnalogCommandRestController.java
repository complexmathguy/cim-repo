...
public class AnalogCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<Void> update(@RequestBody(required = true) UpdateAnalogCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "AnalogController:update() - successfully update Analog - " + exc.getMessage());
    }
  ...
  public void unAssignPositiveFlowIn(
      @RequestBody(required = true) UnAssignPositiveFlowInFromAnalogCommand command) {
    try {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(AnalogCommandRestController.class.getName());
}
