...
public class WindMechIECQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          WindMechIECBusinessDelegate.getWindMechIECInstance()
              .getWindMechIEC(new WindMechIECFetchOneSummary(windMechIECId));
    } catch (Throwable exc) {
  ...
  public List<WindMechIEC> loadAll() {
    ...
    try {
      ...
      if (windMechIECList != null) LOGGER.log(Level.INFO, "successfully loaded all WindMechIECs");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(WindMechIECQueryRestController.class.getName());
}
