...
public class GovHydroWPIDQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovHydroWPIDBusinessDelegate.getGovHydroWPIDInstance()
              .getGovHydroWPID(new GovHydroWPIDFetchOneSummary(govHydroWPIDId));
    } catch (Throwable exc) {
  ...
  public List<GovHydroWPID> loadAll() {
    ...
    try {
      ...
      govHydroWPIDList =
          GovHydroWPIDBusinessDelegate.getGovHydroWPIDInstance().getAllGovHydroWPID();

      if (govHydroWPIDList != null) LOGGER.log(Level.INFO, "successfully loaded all GovHydroWPIDs");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovHydroWPIDQueryRestController.class.getName());
}
