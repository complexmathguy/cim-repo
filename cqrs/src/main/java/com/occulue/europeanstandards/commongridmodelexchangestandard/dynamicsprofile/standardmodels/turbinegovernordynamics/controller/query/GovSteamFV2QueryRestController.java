...
public class GovSteamFV2QueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovSteamFV2BusinessDelegate.getGovSteamFV2Instance()
              .getGovSteamFV2(new GovSteamFV2FetchOneSummary(govSteamFV2Id));
    } catch (Throwable exc) {
  ...
  public List<GovSteamFV2> loadAll() {
    ...
    try {
      ...
      if (govSteamFV2List != null) LOGGER.log(Level.INFO, "successfully loaded all GovSteamFV2s");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovSteamFV2QueryRestController.class.getName());
}
