...
public class AnalogQueryRestController extends BaseSpringRestController {
  ...
  public Analog load(@RequestParam(required = true) UUID analogId) {
    ...
    try {
      entity =
          AnalogBusinessDelegate.getAnalogInstance().getAnalog(new AnalogFetchOneSummary(analogId));
    } catch (Throwable exc) {
  ...
  public List<Analog> loadAll() {
    ...
    try {
      ...
      if (analogList != null) LOGGER.log(Level.INFO, "successfully loaded all Analogs");
    } catch (Throwable exc) {
