...
public class GroundQueryRestController extends BaseSpringRestController {
  ...
  public Ground load(@RequestParam(required = true) UUID groundId) {
    ...
    try {
      entity =
          GroundBusinessDelegate.getGroundInstance().getGround(new GroundFetchOneSummary(groundId));
    } catch (Throwable exc) {
  ...
  public List<Ground> loadAll() {
    ...
    try {
      ...
      if (groundList != null) LOGGER.log(Level.INFO, "successfully loaded all Grounds");
    } catch (Throwable exc) {
