...
public class WindGeneratingUnitCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateWindGeneratingUnitCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          WindGeneratingUnitBusinessDelegate.getWindGeneratingUnitInstance()
              .createWindGeneratingUnit(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateWindGeneratingUnitCommand command) {
    ...
    try {
      ...
      completableFuture =
          WindGeneratingUnitBusinessDelegate.getWindGeneratingUnitInstance()
              .updateWindGeneratingUnit(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "WindGeneratingUnitController:update() - successfully update WindGeneratingUnit - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID windGeneratingUnitId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteWindGeneratingUnitCommand command =
        new DeleteWindGeneratingUnitCommand(windGeneratingUnitId);

    try {
      WindGeneratingUnitBusinessDelegate delegate =
          WindGeneratingUnitBusinessDelegate.getWindGeneratingUnitInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted WindGeneratingUnit with key " + command.getWindGeneratingUnitId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(WindGeneratingUnitCommandRestController.class.getName());
}
