...
public class DCConductingEquipmentQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          DCConductingEquipmentBusinessDelegate.getDCConductingEquipmentInstance()
              .getDCConductingEquipment(
                  new DCConductingEquipmentFetchOneSummary(dCConductingEquipmentId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load DCConductingEquipment using Id " + dCConductingEquipmentId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCConductingEquipmentQueryRestController.class.getName());
}
