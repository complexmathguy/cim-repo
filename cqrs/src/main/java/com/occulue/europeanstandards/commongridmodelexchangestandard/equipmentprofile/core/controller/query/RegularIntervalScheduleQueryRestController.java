...
public class RegularIntervalScheduleQueryRestController extends BaseSpringRestController {
   ...
   */
  @GetMapping("/load")
  public RegularIntervalSchedule load(
    ...
    try {
      entity =
          RegularIntervalScheduleBusinessDelegate.getRegularIntervalScheduleInstance()
              .getRegularIntervalSchedule(
                  new RegularIntervalScheduleFetchOneSummary(regularIntervalScheduleId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load RegularIntervalSchedule using Id " + regularIntervalScheduleId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(RegularIntervalScheduleQueryRestController.class.getName());
}
