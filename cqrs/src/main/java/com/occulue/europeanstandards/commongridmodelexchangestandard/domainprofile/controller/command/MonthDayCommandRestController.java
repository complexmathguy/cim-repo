...
public class MonthDayCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateMonthDayCommand command) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateMonthDayCommand command) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          "MonthDayController:update() - successfully update MonthDay - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID monthDayId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted MonthDay with key " + command.getMonthDayId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(MonthDayCommandRestController.class.getName());
}
