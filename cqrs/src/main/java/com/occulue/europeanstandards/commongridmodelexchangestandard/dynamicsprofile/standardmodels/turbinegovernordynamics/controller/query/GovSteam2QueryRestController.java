...
public class GovSteam2QueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovSteam2BusinessDelegate.getGovSteam2Instance()
              .getGovSteam2(new GovSteam2FetchOneSummary(govSteam2Id));
    } catch (Throwable exc) {
  ...
  public List<GovSteam2> loadAll() {
    ...
    try {
      ...
      if (govSteam2List != null) LOGGER.log(Level.INFO, "successfully loaded all GovSteam2s");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovSteam2QueryRestController.class.getName());
}
