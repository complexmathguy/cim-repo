...
public class ReportingGroupCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateReportingGroupCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          ReportingGroupBusinessDelegate.getReportingGroupInstance().createReportingGroup(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateReportingGroupCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          ReportingGroupBusinessDelegate.getReportingGroupInstance().updateReportingGroup(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "ReportingGroupController:update() - successfully update ReportingGroup - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID reportingGroupId) {
    ...
    try {
      ReportingGroupBusinessDelegate delegate =
          ReportingGroupBusinessDelegate.getReportingGroupInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted ReportingGroup with key " + command.getReportingGroupId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ReportingGroupCommandRestController.class.getName());
}
