...
public class FloatProxyCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateFloatProxyCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          FloatProxyBusinessDelegate.getFloatProxyInstance().createFloatProxy(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateFloatProxyCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          FloatProxyBusinessDelegate.getFloatProxyInstance().updateFloatProxy(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "FloatProxyController:update() - successfully update FloatProxy - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID floatProxyId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted FloatProxy with key " + command.getFloatProxyId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(FloatProxyCommandRestController.class.getName());
}
