...
public class GovCT1QueryRestController extends BaseSpringRestController {
  ...
  public GovCT1 load(@RequestParam(required = true) UUID govCT1Id) {
    ...
    try {
      entity =
          GovCT1BusinessDelegate.getGovCT1Instance().getGovCT1(new GovCT1FetchOneSummary(govCT1Id));
    } catch (Throwable exc) {
  ...
  public List<GovCT1> loadAll() {
    ...
    try {
      ...
      if (govCT1List != null) LOGGER.log(Level.INFO, "successfully loaded all GovCT1s");
    } catch (Throwable exc) {
