...
public class StringProxyCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateStringProxyCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          StringProxyBusinessDelegate.getStringProxyInstance().createStringProxy(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateStringProxyCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          StringProxyBusinessDelegate.getStringProxyInstance().updateStringProxy(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "StringProxyController:update() - successfully update StringProxy - " + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID stringProxyId) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Successfully deleted StringProxy with key " + command.getStringProxyId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(StringProxyCommandRestController.class.getName());
}
