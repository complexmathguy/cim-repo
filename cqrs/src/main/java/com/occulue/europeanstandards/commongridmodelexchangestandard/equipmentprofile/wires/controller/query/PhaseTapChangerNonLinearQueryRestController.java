...
public class PhaseTapChangerNonLinearQueryRestController extends BaseSpringRestController {
   ...
   */
  @GetMapping("/load")
  public PhaseTapChangerNonLinear load(
    ...
    try {
      entity =
          PhaseTapChangerNonLinearBusinessDelegate.getPhaseTapChangerNonLinearInstance()
              .getPhaseTapChangerNonLinear(
                  new PhaseTapChangerNonLinearFetchOneSummary(phaseTapChangerNonLinearId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load PhaseTapChangerNonLinear using Id " + phaseTapChangerNonLinearId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerNonLinearQueryRestController.class.getName());
}
