...
public class GovSteamFV3QueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          GovSteamFV3BusinessDelegate.getGovSteamFV3Instance()
              .getGovSteamFV3(new GovSteamFV3FetchOneSummary(govSteamFV3Id));
    } catch (Throwable exc) {
  ...
  public List<GovSteamFV3> loadAll() {
    ...
    try {
      ...
      if (govSteamFV3List != null) LOGGER.log(Level.INFO, "successfully loaded all GovSteamFV3s");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GovSteamFV3QueryRestController.class.getName());
}
