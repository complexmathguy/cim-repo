...
public class CurrentFlowQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          CurrentFlowBusinessDelegate.getCurrentFlowInstance()
              .getCurrentFlow(new CurrentFlowFetchOneSummary(currentFlowId));
    } catch (Throwable exc) {
  ...
  public List<CurrentFlow> loadAll() {
    ...
    try {
      ...
      if (currentFlowList != null) LOGGER.log(Level.INFO, "successfully loaded all CurrentFlows");
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(CurrentFlowQueryRestController.class.getName());
}
