...
public class VsCapabilityCurveCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateVsCapabilityCurveCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          VsCapabilityCurveBusinessDelegate.getVsCapabilityCurveInstance()
              .createVsCapabilityCurve(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateVsCapabilityCurveCommand command) {
    ...
    try {
      ...
      completableFuture =
          VsCapabilityCurveBusinessDelegate.getVsCapabilityCurveInstance()
              .updateVsCapabilityCurve(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "VsCapabilityCurveController:update() - successfully update VsCapabilityCurve - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID vsCapabilityCurveId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteVsCapabilityCurveCommand command =
        new DeleteVsCapabilityCurveCommand(vsCapabilityCurveId);

    try {
      VsCapabilityCurveBusinessDelegate delegate =
          VsCapabilityCurveBusinessDelegate.getVsCapabilityCurveInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted VsCapabilityCurve with key " + command.getVsCapabilityCurveId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(VsCapabilityCurveCommandRestController.class.getName());
}
