...
public class BoundaryExtensionsQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          BoundaryExtensionsBusinessDelegate.getBoundaryExtensionsInstance()
              .getBoundaryExtensions(new BoundaryExtensionsFetchOneSummary(boundaryExtensionsId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load BoundaryExtensions using Id " + boundaryExtensionsId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(BoundaryExtensionsQueryRestController.class.getName());
}
