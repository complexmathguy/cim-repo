...
public class ENTSOEJunctionCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateENTSOEJunctionCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          ENTSOEJunctionBusinessDelegate.getENTSOEJunctionInstance().createENTSOEJunction(command);
    } catch (Throwable exc) {
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateENTSOEJunctionCommand command) {
    ...
    try {
      ...
      // -----------------------------------------------
      completableFuture =
          ENTSOEJunctionBusinessDelegate.getENTSOEJunctionInstance().updateENTSOEJunction(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "ENTSOEJunctionController:update() - successfully update ENTSOEJunction - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(@RequestParam(required = true) UUID eNTSOEJunctionId) {
    ...
    try {
      ENTSOEJunctionBusinessDelegate delegate =
          ENTSOEJunctionBusinessDelegate.getENTSOEJunctionInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted ENTSOEJunction with key " + command.getENTSOEJunctionId());
    } catch (Throwable exc) {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ENTSOEJunctionCommandRestController.class.getName());
}
