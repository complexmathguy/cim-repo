...
public class EnergySchedulingTypeQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          EnergySchedulingTypeBusinessDelegate.getEnergySchedulingTypeInstance()
              .getEnergySchedulingType(
                  new EnergySchedulingTypeFetchOneSummary(energySchedulingTypeId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING, "failed to load EnergySchedulingType using Id " + energySchedulingTypeId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EnergySchedulingTypeQueryRestController.class.getName());
}
