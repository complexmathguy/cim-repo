...
public class ENTSOEIdentifiedObjectQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ENTSOEIdentifiedObjectBusinessDelegate.getENTSOEIdentifiedObjectInstance()
              .getENTSOEIdentifiedObject(
                  new ENTSOEIdentifiedObjectFetchOneSummary(eNTSOEIdentifiedObjectId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load ENTSOEIdentifiedObject using Id " + eNTSOEIdentifiedObjectId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ENTSOEIdentifiedObjectQueryRestController.class.getName());
}
