...
public class ENTSOETopologicalNodeQueryRestController extends BaseSpringRestController {
    ...
    try {
      entity =
          ENTSOETopologicalNodeBusinessDelegate.getENTSOETopologicalNodeInstance()
              .getENTSOETopologicalNode(
                  new ENTSOETopologicalNodeFetchOneSummary(eNTSOETopologicalNodeId));
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "failed to load ENTSOETopologicalNode using Id " + eNTSOETopologicalNodeId);
      return null;
    }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ENTSOETopologicalNodeQueryRestController.class.getName());
}
