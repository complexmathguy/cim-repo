...
public class ENTSOETopologicalNodeCommandRestController extends BaseSpringRestController {
  ...
  public CompletableFuture<UUID> create(
      @RequestBody(required = true) CreateENTSOETopologicalNodeCommand command) {
    CompletableFuture<UUID> completableFuture = null;
    try {

      completableFuture =
          ENTSOETopologicalNodeBusinessDelegate.getENTSOETopologicalNodeInstance()
              .createENTSOETopologicalNode(command);
    } catch (Throwable exc) {
  ...
  /**
   * Handles updating a ENTSOETopologicalNode. if no key provided, calls create, otherwise calls
   * save
   *
   * @param ENTSOETopologicalNode eNTSOETopologicalNode
  ...
  public CompletableFuture<Void> update(
      @RequestBody(required = true) UpdateENTSOETopologicalNodeCommand command) {
    ...
    try {
      ...
      completableFuture =
          ENTSOETopologicalNodeBusinessDelegate.getENTSOETopologicalNodeInstance()
              .updateENTSOETopologicalNode(command);
      ;
    } catch (Throwable exc) {
      LOGGER.log(
          Level.WARNING,
          "ENTSOETopologicalNodeController:update() - successfully update ENTSOETopologicalNode - "
              + exc.getMessage());
    }
  ...
  public CompletableFuture<Void> delete(
      @RequestParam(required = true) UUID eNTSOETopologicalNodeId) {
    CompletableFuture<Void> completableFuture = null;
    DeleteENTSOETopologicalNodeCommand command =
        new DeleteENTSOETopologicalNodeCommand(eNTSOETopologicalNodeId);

    try {
      ENTSOETopologicalNodeBusinessDelegate delegate =
          ENTSOETopologicalNodeBusinessDelegate.getENTSOETopologicalNodeInstance();

      completableFuture = delegate.delete(command);
      LOGGER.log(
          Level.WARNING,
          "Successfully deleted ENTSOETopologicalNode with key "
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ENTSOETopologicalNodeCommandRestController.class.getName());
}
